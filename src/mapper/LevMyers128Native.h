...
class LevMyers128Native : public LevMyers64 
{
protected:
...
public:
	LevMyers128Native(uint32_t _max_query_len, uint32_t _max_text_len, uint32_t _max_ed);
...
protected:
	virtual void reallocBuffers(uint32_t _max_query_len, uint32_t _max_text_len, int rounding)
	{
		...
		bp128_raw_M = (bp128_t*)alloc_aligned(bp128_raw_ptr_M, (max_text_len + 2 + bp128_n_words) * bp128_n_words * sizeof(bp128_t), sizeof(simd128_t));

		for (uint32_t i = 0; i < max_text_len + 2; ++i)
			bp128_M[i] = &bp128_raw_M[i * bp128_n_words];

		for (uint32_t i = 0; i < bp128_n_words; ++i)
		{
			//		bp128_M[0][i].VP = ~(0ull);
			bp128_M[0][i].VP = _mm_set_epi64x(~(0ull), ~(0ull));
			//		bp128_M[0][i].VN = 0;
			bp128_M[0][i].VN = _mm_set_epi64x(0, 0);

			//		bp128_M[0][i].HN = 0;
			bp128_M[0][i].HN = _mm_set_epi64x(0, 0);
			//		bp128_M[0][i].HP = 0;
			bp128_M[0][i].HP = _mm_set_epi64x(0, 0);
			//		bp128_M[0][i].D0 = ~(0ull);
			bp128_M[0][i].D0 = _mm_set_epi64x(~(0ull), ~(0ull));
		}
