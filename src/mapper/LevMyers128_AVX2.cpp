...
// ************************************************************************************
template<>
LevMyers128<instruction_set_t::avx2>::LevMyers128(uint32_t _max_query_len, uint32_t _max_text_len, uint32_t _max_ed)
	:
...
// ************************************************************************************
template<>
bool LevMyers128<instruction_set_t::avx2>::dynamicProgramming(
...
// ************************************************************************************
template<>
ref_pos_t LevMyers128<instruction_set_t::avx2>::getExtCigar(uchar_t *ext_cigar, uchar_t* tmp_ref_sequence, uchar_t* tmp_read_sequence, uchar_t* quality,
...
//	float min_qual = '!';
//	float max_qual = '~';
//	float qual_range = max_qual - min_qual;
	...
	while (read_pos && pos) {
		if (tmp_read_sequence[read_pos] == tmp_ref_sequence[pos - 1]) {
			// match
			inIndel = false;
		//	float scale = ((float)quality_reader[read_pos] - min_qual) / qual_range;
		//	scale = scale / 3 + 0.33;
			affine_score += scoring.match; 
			--read_pos;
			--pos;
			ext_cigar[cigar_pos++] = '.';
		}
...
template<>
void LevMyers128<instruction_set_t::avx2>::save(const std::string& filename)
{
...
/*********************************************************************************************/
template<>
LevMyers128Native<instruction_set_t::avx2>::LevMyers128Native(uint32_t _max_query_len, uint32_t _max_text_len, uint32_t _max_ed)
	:
	LevMyers64(_max_query_len, _max_text_len, _max_ed, 2),
	genome_prefetch(nullptr),
	bp128_raw_ptr_M(nullptr),
...
template<>
bool LevMyers128Native<instruction_set_t::avx2>::dynamicProgramming(
	ref_pos_t ref_pos, uint32_t max_distance_in_ref, uint32_t max_mate_edit_distance, ref_pos_t &pos, uint32_t &edit_distance) {
...
template<>
ref_pos_t LevMyers128Native<instruction_set_t::avx2>::getExtCigar(uchar_t *ext_cigar, uchar_t* tmp_ref_sequence, uchar_t* tmp_read_sequence, uchar_t* quality,
	ref_pos_t pos, uint32_t edit_dist, uint32_t seq_len, genome_t dir, const scoring_t &scoring, double& affine_score, uint32_t& num_events) {
...
template<>
void LevMyers128Native<instruction_set_t::avx2>::save(const std::string& filename) {
