...
size_t UnixMemoryTools::processCurrentWorkingSet()
{
	size_t size = 0;
	...
	if (file) {
		unsigned long tmp = 0;
		unsigned long vm = 0;
		fscanf(file, "%ul", &tmp);  // Just need the 2nd num: vm size
		fscanf(file, "%ul", &vm);  // Just need the 2nd num: vm size
		fclose(file);
...
size_t UnixMemoryTools::processPeakWorkingSet()
{
	// Currently the same as processPeakVirtual
	return UnixMemoryTools::processPeakVirtual();
}
